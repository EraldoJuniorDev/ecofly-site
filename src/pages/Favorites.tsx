...
const Favorites = () => {
  ...
  const handleRemoveFavorite = (itemId: number, itemName: string) => {
    const confirmed = window.confirm(`Tem certeza que deseja remover "${itemName}" dos favoritos?`);
    if (!confirmed) return;
  ...
  const handleClearAll = () => {
    ...
    const confirmed = window.confirm('Tem certeza que deseja remover todos os itens dos favoritos?');
    if (!confirmed) return;
      ...
      <div className="container px-4 py-16">
        <div className="text-center max-w-lg mx-auto">
          ...
          <p className="text-muted-foreground mb-8">
            Você ainda não adicionou nenhum produto aos favoritos. Explore nossa loja e salve os produtos que mais gostar!
          </p>
          <Link to="/catalogo">
  ...
  return (
    <div className="container px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold eco-text-gradient">Meus Favoritos</h1>
        ...
        {favorites.map(item => (
          <Card key={item.id} className="group overflow-hidden hover-subtle animate-fade-in-up">
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <h3 className="font-semibold text-lg line-clamp-2 group-hover:text-primary transition-colors duration-200">{item.name}</h3>
              <p className="text-sm text-muted-foreground line-clamp-2">{item.description}</p>
              <div className="flex gap-2">
                <Button onClick={() => handleWhatsApp(item.name)} className="flex-1 eco-gradient text-white btn-smooth" size="sm">
                  <MessageCircle className="w-4 h-4 mr-2" />
                  WhatsApp
                </Button>
                <Button onClick={() => handleRemoveFavorite(item.id, item.name)} variant="outline" size="sm" className="text-destructive hover:text-destructive border-destructive/20 hover:border-destructive">
