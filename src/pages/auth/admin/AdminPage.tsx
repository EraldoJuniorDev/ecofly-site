...
const AdminPage: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      toast({ description: err?.message || "Erro ao carregar produtos" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      if (file.size > 5 * 1024 * 1024) {
        toast({ description: `A imagem "${file.name}" excede 5MB.` });
        return;
      }
    }
  ...
  // --- Add validation ---
  const validateForm = (data: typeof formData | typeof editForm | typeof quickEditForm) => {
    if (!data.name.trim() || !data.category || !data.description.trim()) {
      toast({ description: "Por favor, preencha todos os campos obrigatórios." });
      return false;
    }
    if ('images' in data && data.images.length === 0) {
      toast({ description: "Por favor, selecione pelo menos uma imagem." });
      return false;
    }
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      toast({ description: err?.message || "Erro ao adicionar produto" });
    } finally {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const slug = await generateUniqueSlug(editForm.name);
      ...
      toast({ description: err?.message || "Erro ao atualizar produto" });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleQuickEditSubmit = async () => {
    ...
    if (!validateForm(quickEditForm)) return;
    setShowQuickEditConfirmDialog(false);
    setShowQuickEditDialog(false);
    setIsSubmitting(true);
    try {
      const slug = await generateUniqueSlug(quickEditForm.name);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast({ description: err?.message || "Erro ao excluir produto" });
    } finally {
      setIsSubmitting(false);
    }
  };
                ...
                <Tabs value={activeTab} onValueChange={(v) => setActiveTab(String(v))} className="space-y-6">
                    ...
                    >
                      <div className="p-4 sm:p-8">
                        <div className="flex items-center gap-3 mb-8">
                          ...
                          <div>
                            <h2 className="text-xl sm:text-2xl font-bold">Editar Produtos</h2>
                            <p className="text-sm sm:text-base text-muted-foreground">Selecione um produto para editar suas informações</p>
                          </div>
                        </div>
                      ...
                      <DialogContent className="max-w-[500px]">
                        <DialogHeader>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Tem certeza que deseja salvar as alterações no produto "{editingProduct?.name}"?
                          </p>
                        </DialogHeader>
                        <DialogFooter className="flex flex-col sm:flex-row justify-end gap-3">
                          <Button
                  ...
                  <TabsContent value="delete">
                    ...
                    >
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                          <h2 className="text-xl sm:text-2xl font-bold">Excluir Produtos</h2>
                          <p className="text-sm sm:text-base text-muted-foreground">Selecione um produto para excluir permanentemente</p>
                        </div>
                      </div>
                      {isLoading ? (
                        <p className="text-muted-foreground">Carregando produtos...</p>
                  ...
                  <TabsContent value="list">
                    ...
                    >
                      ...
                      {isLoading ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {[...Array(6)].map((_, i) => (
                            <div key={i} className="bg-white/70 dark:bg-background rounded-2xl p-6 animate-pulse" />
                          ))}
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                          {listFilteredProducts.map((product) => (
                            <motion.div
                              ...
                              initial={{ opacity: 0, y: 8 }}
                              animate={{ opacity: 1, y: 0 }}
                              className="bg-white/70 dark:bg-background rounded-2xl border border-slate-200/60 dark:border-slate-700/40 overflow-hidden shadow-sm"
                            >
                              <div className="relative h-48 overflow-hidden">
                                ...
                                />
                                <div className="absolute top-3 right-3 bg-white/90 dark:bg-background px-3 py-1 rounded-full text-xs font-medium text-slate-700 dark:text-slate-200">
                                  {product.category}
                                </div>
                              </div>
                              <div className="p-4">
                                ...
                                <div className="flex justify-center items-center mt-4">
                                  <div className="flex items-center gap-5">
                                    ...
                                    >
                                      ...
                                      <span className="ml-2 sm:inline">Excluir</span>
                                    </Button>
                                  </div>
                                </div>
                              </div>
                    ...
                    <Dialog open={showQuickEditConfirmDialog} onOpenChange={setShowQuickEditConfirmDialog}>
                      <DialogContent className="max-w-[500px]">
                        <DialogHeader>
                          <DialogTitle>Confirmar Edição Rápida</DialogTitle>
                          <p className="text-sm text-muted-foreground">
                            Tem certeza que deseja salvar as alterações no produto "{quickEditProduct?.name}"?
                          </p>
                        </DialogHeader>
                        <DialogFooter className="flex flex-col sm:flex-row justify-end gap-3">
                          <Button
